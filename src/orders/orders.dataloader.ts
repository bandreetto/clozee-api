...
export class OrdersLoader extends DataLoader<string, Order> {
  constructor(private readonly ordersService: OrdersService) {
    super((ids: string[]) =>
      this.ordersService.findManyByIds(ids).then(orders => reconciliateByKey('_id', ids, orders)),
    );
  }
  ...
  byUser = new DataLoader<string, Order[]>((userIds: string[]) =>
    this.ordersService
      ...
      .then(orders => userIds.map(userId => orders.filter(order => order.buyer === userId))),
  );
}
