...
export class PostsLoader extends DataLoader<string, Post> {
  constructor(private readonly postsService: PostsService) {
    super((ids: string[]) => this.postsService.findManyByIds(ids).then(posts => reconciliateByKey('_id', ids, posts)));
  }

  byUser = new DataLoader<string, Post[]>((userIds: string[]) =>
    this.postsService
      .findManyByUsers(userIds)
      .then(posts => userIds.map(userId => posts.filter(post => post.user === userId))),
  );
}
